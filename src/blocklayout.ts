...
export class Std140Block {
  ...
  private _ensureBufferSize(wantMoreByteSize: number): void {
  ...
  addUint32(v: number, name?: string): void {
  ...
  addVec2(x: number, y: number): void {
  ...
  addVec3(xyz: vec3): void {
    this.addVec4(xyz[0], xyz[1], xyz[2], 0);
  }

  addVec4(x: number, y: number, z: number, w: number): void {
  ...
  addUvec3(x: number, y: number, z: number): void {
    this.addUvec4(x, y, z, 0);
  }

  addUvec4(x: number, y: number, z: number, w: number): void {
    const align = 16;
...
export class Std430Block {
  ...
  addColumnMajorMat3(mat: [vec3, vec3, vec3]): void {
    const COLS = 3, ROWS = 3;
